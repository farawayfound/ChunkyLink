...
if grep -q '^OLLAMA_NUM_CTX=' "$ENV_FILE"; then
  sed -i 's/^OLLAMA_NUM_CTX=.*/OLLAMA_NUM_CTX=64000/' "$ENV_FILE"
else
  echo 'OLLAMA_NUM_CTX=64000' >> "$ENV_FILE"
fi
...
echo "    Worker num_ctx is pushed from M1 backend Redis; ensure WORKER_OLLAMA_NUM_CTX=64000 in backend .env or Admin."
