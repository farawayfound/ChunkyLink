...
4. Force-align nanobot worker/runtime model settings to **`gemma4:26b`** + **`131072`** in **`.env`**, **`.env.nanobot`**, and (when present) **`data/admin_config.json`**.
...
| `scripts/deploy_chunkylink.sh` | Pull (or reset with **`DEPLOY_RESET_HARD=1`**), dependencies, optional frontend build, align worker/runtime Ollama to **`gemma4:26b`** + **`131072`**, `chown`, **`systemctl restart chunkylink`** — on **nanobot**, prefer **`DEPLOY_RESET_HARD=1`** ([§ 2](#2-deploy-on-the-linux-server-nanobot)). |
